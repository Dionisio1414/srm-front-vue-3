<template>
  <directories-table
    ...
    :storage-key="STORAGE_KEYS.GENERAL_COUNTRIES_COLUMNS"
    :fields="GENERAL_COUNTRIES_FIELDS"
  >
    <template #waysFilter="{ exportData, updatedFilter, resetFiltersHandler, filters }">
      ...
      <prime-button
        ...
        style="width: 2.8125rem; height: 2.8125rem"
        @click.stop="exportData('countries')"
      >
      ...
      >
        ...
        <span class="text-lg">
          {{ $t('directories.buttons.newCountry') }}
        </span>
      </prime-button>
    </template>
  </directories-table>
...
const { isLoading, data, getData } = useGetData('allCountries', infoStore.getAllCountries);
