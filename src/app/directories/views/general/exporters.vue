<template>
  <directories-table
    ...
    :storage-key="STORAGE_KEYS.GENERAL_EXPORTERS_COLUMNS"
    :fields="GENERAL_EXPORTERS_FIELDS"
  >
    <template #waysFilter="{ exportData, updatedFilter, resetFiltersHandler, filters }">
      ...
      <prime-button
        ...
        style="width: 2.8125rem; height: 2.8125rem"
        @click.stop="exportData('exporters')"
      >
      ...
      >
        ...
        <span class="text-lg">
          {{ $t('directories.buttons.newExporter') }}
        </span>
      </prime-button>
    </template>
  </directories-table>
...
const { isLoading, data, getData } = useGetData('exporters', infoStore.getExporters);
