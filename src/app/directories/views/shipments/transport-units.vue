<template>
  <directories-table
    ...
    :storage-key="STORAGE_KEYS.SHIPMENTS_TRANSPORT_UNITS_COLUMNS"
    :fields="SHIPMENTS_TRANSPORT_UNITS"
  >
    <template #waysFilter="{ exportCSV, updatedFilter, resetFiltersHandler, filters }">
      <prime-button
        type="button"
      ...
      <prime-button
        ...
        style="width: 2.8125rem; height: 2.8125rem"
        @click.stop="exportCSV('csv', 'transport-units')"
      >
      ...
      >
        ...
        <span class="text-lg">
          {{ $t('directories.buttons.newTUType') }}
        </span>
      </prime-button>
...
const { isLoading, data, getData } = useGetData('unitTypes', infoStore.getUnitTypes);
