<template>
  <directories-table
    ...
    :storage-key="STORAGE_KEYS.SHIPMENTS_STATUSES_COLUMNS"
    :fields="SHIPMENTS_STATUSES"
  >
    <template #waysFilter="{ exportData, updatedFilter, resetFiltersHandler, filters }">
      ...
      <prime-button
        ...
        style="width: 2.8125rem; height: 2.8125rem"
        @click.stop="exportData('statuses')"
      >
      ...
      >
        ...
        <span class="text-lg">
          {{ $t('directories.buttons.newStatus') }}
        </span>
      </prime-button>
    </template>
  </directories-table>
...
const { isLoading, data, getData } = useGetData(
  'shipmentsStatuses',
  infoStore.getShipmentsStatuses
);
